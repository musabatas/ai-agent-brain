...
import { generateMcpConfig, mcpConfigToJson } from '@/lib/mcp-config';
...
export default function McpConfigPage() {
  ...
  const mcpConfig = mcpConfigToJson(
    generateMcpConfig({ origin, projectSlug: slug }),
  );
  ...
  const CopyBtn = ({ textKey, text }: { textKey: string; text: string }) => (
    <button
      ...
      className="flex items-center gap-1 text-xs text-muted-foreground/50 hover:text-foreground transition-colors cursor-pointer"
    >
      {copied === textKey ? (
        <>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {/* 1. Server Config */}
      <div className="bg-card border hover:bg-accent/50 transition-colors rounded-xl p-5 space-y-3">
        ...
        <pre className="font-mono bg-zinc-950 text-zinc-300 p-4 rounded-lg text-xs overflow-x-auto border border-zinc-800 leading-relaxed">
          {mcpConfig}
        </pre>
        <p className="text-xs text-muted-foreground/50">
          Add to <code className="font-mono">.mcp.json</code> (Claude Code) or
          MCP settings (Cursor). Replace{' '}
          <code className="font-mono text-amber-400/70">YOUR_API_KEY</code> with
          a key from{' '}
          <Link
            href="/org/api-keys"
      ...
      <div className="bg-card border hover:bg-accent/50 transition-colors rounded-xl p-5 space-y-3">
        ...
        <div className="text-[12px] text-muted-foreground/70 space-y-2 leading-relaxed">
          <p>
            The <code className="font-mono text-primary/70">X-Project-Slug</code>{' '}
            header sets the default project for all MCP tool calls. Tools can
            still override it by passing an explicit{' '}
            <code className="font-mono text-primary/70">project</code> parameter.
          </p>
          <p>
