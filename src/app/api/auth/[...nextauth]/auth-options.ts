  ...
  providers: [
    ...
    GoogleProvider({
      ...
      async profile(profile) {
        ...
        // Create user (org creation handled by onboarding flow)
  ...
  callbacks: {
    async jwt({
      ...
      if (trigger === 'update' && session?.user) {
        ...
        if (user && user.roleId) {
          ...
          // Resolve default org (first org the user owns)
          if (user.defaultOrgId) {
            ...
            token.defaultOrgId = membership?.orgId ?? null;
          }
        }
      }
    ...
    async session({ session, token }: { session: Session; token: JWT }) {
      if (session.user) {
        ...
        session.user.defaultOrgId = token.defaultOrgId;
      }
      return session;
    },
  },
